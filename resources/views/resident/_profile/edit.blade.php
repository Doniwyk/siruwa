...
<div class="bg-white rounded-2xl flex p-9 mb-6">
    <form action="{{ route('resident.profil.update') }}" class="flex sm:flex-col lg:flex-row gap-9 w-full " id="update-personal-data" method="POST"
...
<div class="bg-white p-9 rounded-2xl mb-6">
    ...
    <form id="update-password" action="{{ route('resident.profil.changePassword') }}" method="post" class="flex sm:flex-col gap-9">
        @csrf
        @method('PUT')
...
<!-- Modal -->
<div class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-10 hidden" id="success-modal">
    <div class="flex flex-col bg-white rounded-2xl p-10 gap-9">        
        ...
        <div class="flex justify-center">
            <button type="button" id="close-modal-button" class="w-52 bg-secondary text-stone-50 font-semibold py-3 rounded-2xl flex justify-center items-center">
                Baik
            </button>
        </div>
    </div>
