<div class="bg-secondary px-16 py-8 text-white">
    <div class="flex justify-between">
        <div class="flex flex-col gap-4">
            <div class="text-5xl font-extrabold">>///<</div>
            <div>Desa Sumberejo, Kota Batu, Jawa Timur</div>
            <div class="flex gap-4">
                <img src="{{ asset('..\assets\icons\phone.svg') }}" class="w-6">
                <div class="text-2xl font-semibold">+6283845734645</div>
            </div>
        </div>
        <div class="flex flex-col gap-4 text-base font-medium">
            <span class="text-xl font-semibold">Halaman Utama</span>
            <a href=""><span>Beranda</span></a>
            <a href=""><span>Menu</span></a>
            <a href=""><span>Berita</span></a>
            <a href=""><span>Agenda</span></a>
            <a href=""><span>Kepengurusan</span></a>
        </div>
        <div class="flex flex-col gap-4 text-base font-medium">
            <span class="text-xl font-semibold">Menu</span>
            <a href=""><span>Edit Profil</span></a>
            <a href=""><span>Data Penduduk</span></a>
            <a href=""><span>Pengajuan Dokumen</span></a>
            <a href=""><span>Iuran</span></a>
            <a href=""><span>Statistik Data</span></a>
        </div>
        <div class="flex flex-col gap-4 text-base font-medium">
            <span class="text-xl font-semibold">Aspirasi</span>
            <a href=""><span>Form Aspirasi</span></a>
        </div>
    </div>
    <div class="text-xs font-normal mt-3">Copyright &copy; 2024 SiUwU - Politeknik Negeri malang</div>
</div>
